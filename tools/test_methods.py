...
def test_comput_embeddings(self, spatial_net, test_batch=1025, mode='val'):
    ...
    while j < data_length:
        ...
        if mode != 'val':
            if (j % 1000) == 0:
                print(j)
    print('embedding time of {} trajectories: {}'.format(data_length, time.time() - s))
...
def validate_model(self, traj_embeddings, rank_info):
    ...
    for anchor_traj in cluster_center:
        ...
        sorted_test_id = [i[0] for i in sorted_test_distance][:5]
...
def final_test_model(self, traj_embeddings, print_batch=10, similarity=True, r10in50=False):
    test_range = range(config.datalength, config.full_datalength)
    top_10_count = 0
    top_50_count = 0
