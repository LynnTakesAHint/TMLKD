...
def distance_sampling(distance, train_seq_len, index, mail_pre_degree):
...
def negative_distance_sampling(distance, train_seq_len, index, mail_pre_degree, MEEtrain_distance=None):
    index_dis = distance[index]
    pre_sort = [np.exp(-i * mail_pre_degree) for i in index_dis[:train_seq_len]]
    ...
    importance = np.array(importance)
    # print importance
    # while len(sample_index) < config.sampling_num // 2: # 带易错采样
    while len(sample_index) < config.sampling_num:  # 原采样
        a = np.random.uniform()
        idx = np.where(importance > a)[0]
        if len(idx) == 0:
            continue  # sample_index.append(train_seq_len - 1)
        elif ((idx[0] - 1) not in sample_index) & (not ((idx[0] - 1) == index)):
            sample_index.append(idx[0] - 1)
    sorted_sample_index = []
    for i in sample_index:
