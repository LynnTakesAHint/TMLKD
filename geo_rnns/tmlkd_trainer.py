...
import datetime
from geo_rnns.t3s_model import T3S_Network # under construction
...
class TMLKD_Trainer(object):
    ...
    def data_prepare(self, griddatapath=config.griddatapath,
        ...
        self.distance = distance  # 取出有用的那部分
        # 处理embedding
        ...
        if config.data_name == 'sroma':
            if config.distance_type == 'hausdorff':
                extra_range = range(239, 245)
            elif config.distance_type == 'sspd':
                extra_range = range(244, 247)
            elif config.distance_type == 'discret_frechet':
                extra_range = []
            else:
                extra_range = range(1, 200, 10)
            for idx in extra_range:
                self.anchor_indice.append(self.anchor_indice[idx])
        self.train_indice = self.anchor_indice[:config.train_size]
        self.val_indice = self.anchor_indice[config.train_size:]
        self.final_train_indice = self.get_indices()


    def get_indices(self):
        lst = self.train_indice
        idx = 0
        while len(lst) < 2000:
            if idx in self.anchor_indice:
                idx += 1
                continue
            lst.append(idx)
            idx += 1
    ...
    def get_base_model(self):
        if config.base_model == 'T3S':
            return T3S_Network(target_size=self.target_size, batch_size=self.batch_size,
                               sampling_num=self.sampling_num)
        elif config.base_model == 'neutraj':
            return NeuTraj_Network(4, self.target_size, config.grid_size, config.batch_size, config.sampling_num, stard_LSTM=False, incell=config.incell)
        else:
            return T3S_Network(target_size=self.target_size, batch_size=self.batch_size,
                               sampling_num=self.sampling_num)

    def trained_model_eval(self, load_model=None):
        spatial_net = self.get_base_model()
        spatial_net.cuda()
    ...
    def get_optimizer(self, model):
        ...
        opt = torch.optim.Adam([
            {'params': params_to_opt[0], 'lr': config.learning_rate}])
        return opt

    def TMLKD_train(self, print_batch=40, print_test=300, save_model=True, load_model=None, save_tag=""):
        global end
        spatial_net = self.get_base_model()
        optimizer = self.get_optimizer(spatial_net)
        my_loss = Total_Loss()
        spatial_net.cuda()
        my_loss.cuda()
        ...
        for epoch in range(config.epochs):
            ...
            if save_model and acc1 > best_top10_acc:
                ...
                save_model_name = (config.data_folder+"model/tmlkd/" + str(config.data_type) + "_" + x.strftime("%d") + "_" + x.strftime(
                        "%H") + "_" + str(config.distance_type) + "_" + str(config.d) +"_" + str(config.prob_rho)+"_" +
                                       str(config.weak_threshold)+"_"
                                       + "_best_model.h5")
